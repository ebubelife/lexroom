...
<body x-data="{ sidebarOpen: false }">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-56 flex flex-col transition-transform duration-300 lg:translate-x-0"
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            ...
            @if($navAdmin->hasAbility('admin.support.view'))
            <a href="{{ route('admin.support.index') }}"
               class="nav-link {{ request()->routeIs('admin.support.*') ? 'active' : '' }}">
                ...
                @if($openCount > 0)
                    <span class="ml-auto text-xs px-1.5 py-0.5 rounded-full font-semibold" style="background: rgba(234,179,8,0.2); color: #eab308;">{{ $openCount }}</span>
                @endif
            </a>
            @endif
            ...
            @if($navAdmin->hasAbility('admin.newsletter.view'))
