    ...
    <div style="max-width: 480px; width: 100%; margin: 0 auto; padding: 20px;">
        <div style="background: white; border-radius: 16px; padding: 40px; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
            ...
            <p style="color: #6B6B68; text-align: center; font-size: 14px; margin-bottom: 30px;">
                Pay to activate your mediation session.
            </p>

            <div style="background: #F5EDD6; border-radius: 10px; padding: 16px; margin-bottom: 24px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: #6B6B68; font-size: 14px;">Case ID</span>
                    <span style="color: #0D1B2A; font-weight: 500; font-size: 14px;">{{ $room->case_id }}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: #6B6B68; font-size: 14px;">Payment Type</span>
                    <span style="color: #0D1B2A; font-weight: 500; font-size: 14px;">{{ ucfirst($room->payment_type) }}</span>
                </div>
                <div style="border-top: 1px solid #E8C96A; margin: 12px 0;"></div>
                <div style="display: flex; justify-content: space-between;">
                    <span style="color: #0D1B2A; font-weight: 600;">{{ $label }}</span>
            ...
            <a href="{{ route('payment.checkout', $room->id) }}"
               style="display: block; background: #C9A84C; color: white; padding: 14px; text-align: center; border-radius: 10px; font-weight: 600; text-decoration: none; font-size: 16px;">
                Pay {{ \App\Models\PlatformSetting::currencySymbol() }}{{ number_format($amount, 2) }} & Continue
            </a>

            <p style="text-align: center; font-size: 12px; color: #6B6B68; margin-top: 16px;">
                Secured by Stripe. 
                @if($room->payment_type === 'split')
                    Party B will be notified to pay their share after your payment.
                @else
                    Party B will be invited to join after your payment.
                @endif
            </p>
        </div>
    </div>
